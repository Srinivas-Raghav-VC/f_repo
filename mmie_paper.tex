...
Large language models learn many languages in a shared internal space. This makes “deleting” a single language---especially a low-resource one like Hindi---surprisingly hard. In this paper we ask a simple question: \emph{can we turn off Hindi in a multilingual model without breaking everything else?}

We study this question on Qwen2.5-1.5B-Instruct using a small Hindi/English dataset with related languages (Urdu, Punjabi, Bengali). First, we locate mid-layer “Hindi” directions using a sparse autoencoder (SAE) trained on hidden states. Next, we try two kinds of interventions: (1) directly suppressing those SAE features during inference, and (2) training small LoRA/ReFT adapters on top of them. Finally, we measure the effects on Hindi, English, and related languages using one evaluation suite: extraction strength, perplexity, cross-lingual leakage, and simple privacy metrics.

In our case study we can reduce Hindi, but we cannot cleanly remove it: whenever we push Hindi down, we either leave residual Hindi or harm English and the related languages. This supports the view that Hindi lives in a shared mid-layer subspace rather than in a neat “Hindi module”. We release code and evaluation scripts so others can repeat the same analysis on larger models.
\end{abstractbox}
...
\paragraph{Our approach (in three steps).} Our method is intentionally simple:

\begin{enumerate}[leftmargin=*, itemsep=3pt]
    \item \textbf{Find where Hindi lives.} We scan layers and identify mid layers where Hindi vs. English is easiest to separate.
    \item \textbf{Learn Hindi features.} At those layers we train sparse autoencoders (SAEs) on activations, then select features that fire on Hindi in both Devanagari and Roman script but not on Devanagari gibberish.
    \item \textbf{Turn those features down and measure.} We either (a) gate those SAE features during inference, or (b) combine gating with small LoRA/ReFT adapters, then measure what happens to Hindi, English, and related languages.
\end{enumerate}
...
\begin{figure}[htbp]
  \centering
  % Placeholder: layer-wise ES or selection scores
  \fbox{\parbox{0.9\linewidth}{\centering
    \textit{Plot placeholder: layer-wise Hindi vs. English separability and chosen layers.}
  }}
  \caption{\textbf{Layer selection view.} To be replaced by a plot showing ES or semantic scores per layer and the selected mid layers.}
  \label{fig:layer-selection}
\end{figure}

\begin{figure}[htbp]
  \centering
  % Placeholder: forget vs retain trade-off
  \fbox{\parbox{0.9\linewidth}{\centering
    \textit{Plot placeholder: Hindi ES vs. English PPL for Base, SAE-gate, LoRA+SAE, GRUN+SAE.}
  }}
  \caption{\textbf{Forget--retain trade-off.} To be replaced by a scatter or bar plot summarizing ES and PPL for each arm.}
  \label{fig:tradeoff}
\end{figure}
